...
# class weight_quantize_fn_v1(nn.Module):
#     def __init__(self, w_bit):
#         super(weight_quantize_fn_v1, self).__init__()
#         self.wbit = w_bit
#         assert self.wbit <= 8 or self.wbit == 32
...
#             weight = torch.tanh(x) # 현재 weigh의 분포도
            
#             E_cen = torch.mean(torch.abs(y)).detach()
#             weight_cen = torch.tanh(y)
#             # weight = weight / 2 / torch.max(torch.abs(weight_cen)) + 0.5
#             weight_cen = weight_cen / 2 / torch.max(torch.abs(weight_cen)) + 0.5
#             # weight_q = 2 * qfn.apply(weight, self.wbit) - 1
#             weight_q = 2 * qfn.apply(weight_cen, self.wbit) - 1
#             weight_q = weight_q * E_cen

#             with torch.no_grad():
#                 if pw == 5:
#                     weight[:,:,2:5, 2:5] = weight_q
#                 elif pw == 4:
#                     weight[:,:,1:4, 1:4] = weight_q
...
class SwitchedConv2d_update_ours(Conv2d_Q_) :
    ...
    def forward(self, input) :
        ...
        if nofwindows == 4 :
            ...
            ad_weight1 = slicing_conv(idx[0], windows, self.pw, weight_q, self.pat[0])
            mask1_out1 = F.conv2d(input, ad_weight1, self.bias, self.stride, self.padding, self.dilation, self.groups)

            ad_weight2 = slicing_conv(idx[1], windows, self.pw, weight_q, self.pat[1])
            ...
            ad_weight3 = slicing_conv(idx[2], windows, self.pw, weight_q, self.pat[2])
            mask1_out3 = F.conv2d(input, ad_weight3, self.bias, self.stride, self.padding, self.dilation, self.groups)

            ad_weight4 = slicing_conv(idx[3], windows, self.pw, weight_q, self.pat[3])
            ...
            ad_weight5 = slicing_conv(idx[4], windows, self.pw, weight_q, self.pat[4])
            mask1_out5 = F.conv2d(input, ad_weight5, self.bias, self.stride, 1, self.dilation, self.groups)

            ad_weight6 = slicing_conv(idx[5], windows, self.pw, weight_q, self.pat[5])
            ...
            ad_weight7 = slicing_conv(idx[6], windows, self.pw, weight_q, self.pat[6])
            mask1_out7 = F.conv2d(input, ad_weight7, self.bias, self.stride, 1, self.dilation, self.groups)

            ad_weight8 = slicing_conv(idx[7], windows, self.pw, weight_q, self.pat[7])
            mask1_out8 = F.conv2d(input, ad_weight8, self.bias, self.stride, 1, self.dilation, self.groups)

            ad_weight9 = slicing_conv(idx[8], windows, self.pw, weight_q, self.pat[8])
